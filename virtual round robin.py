...
while flag == 0:
    ...
    for c1 in range(n):
        if r_burst_time[c1] > 0:
            ...
            flag = 1
        if current_time == re_arrival[c1-1]:
            if r_burst_time[c1-1] > quantum:
                waiting_time[c1] += quantum
            elif r_burst_time < quantum:
                waiting_time[c1] += r_burst_time[c1-1]
